...
local WEBHOOK_URL = "https://webhook.lewisakura.moe/api/webhooks/1377287698733072485/teSYs9krSFopTsoadjM40XC72Cf1FM_avTgDBgSJjjTHjqJ4ROTg81Xle6PYWl54r7wH"
...
local FlyHackDetector = true
local MAX_FLY_TIME = 1

local CFramePositionDetector = true --[[ This option will prevent a cheater from using Speed,Teleportation, and similar cheats.
	...
	player.CharacterAdded:Connect(function(character)
		...
		local function executeKickProcedure(playerToKick, kickReasonCode, diagnosticMessage)
			...
			if DiscordWebhook then
				...
				if successAvatar then playerAvatarUrl = urlOrErrAvatar
		...
		if FlyHackDetector then
			...
			RunService.PreSimulation:Connect(function(dt)
				...
				if #Animator:GetPlayingAnimationTracks() == 0 then
					Suspicious = true
					FlyingTime += dt
					if FlyingTime >= MAX_FLY_TIME then
						Detected = true
						FlyingTime = 0
				...
				if oldState == Enum.HumanoidStateType.Freefall and newState == Enum.HumanoidStateType.Jumping then
					if tick() - (playerFallData.lastLandedTime or 0) > 0.25 then 
						if tick() - lastMidAirJumpSetback > midAirJumpSetbackCooldown then
							...
							print(string.format("Player %s: %s. lastLandedTime: %.2f, tick(): %.2f", player.Name, diagnostic, (data.lastLandedTime or 0), tick()))
							...
							return -- Important to stop further processing for this specific state change if a mid-air jump was handled
						else
							print(string.format("Player %s: Mid-air jump detected but on cooldown. lastLandedTime: %.2f", player.Name, (data.lastLandedTime or 0)))
						end
					else
		...
		if CFramePositionDetector then
			...
			while character and character.Parent and rootPart and rootPart.Parent and humanoid and humanoid.Health > 0 and player and player.Parent do
				...
				if speedToEvaluate > currentMaxAllowedSpeedToUse + 2 then 
					...
					print(player.Name .. " violated speed. Speed: " .. string.format("%.2f", speedToEvaluate) ..
						", MaxAllowed: " .. string.format("%.2f", currentMaxAllowedSpeedToUse) ..
						", State: " .. currentHumanoidStateType.Name ..
					...
					if VIOLATION_COUNT >= MAX_VIOLATIONS_BEFORE_KICK then
						...
						local diagnosticText = string.format("Speed: %.2f studs/s (Max: %.2f), State: %s. Accumulated violations: %d/%d.",
							speedToEvaluate, currentMaxAllowedSpeedToUse, currentHumanoidStateType.Name, VIOLATION_COUNT, MAX_VIOLATIONS_BEFORE_KICK)

						executeKickProcedure(player, fullKickCode, diagnosticText)
						break 
					else
						local currentOrientationY = select(2, rootPart.CFrame:ToOrientation())
...
KickEvent.OnServerEvent:Connect(function(player, kickreason)
	if DiscordWebhook then
		local playerAvatarUrl = getPlayerAvatarUrl(player.UserId)
		local embedData = {
			["title"] = "Player Kicked",
			["description"] = "**" .. player.Name .. "** (ID: " .. player.UserId .. ") has been kicked by the anti-cheat.",
			["color"] = 16711680, -- Red
			["fields"] = {
				{
...
BanEvent.OnServerEvent:Connect(function(player, banreason)
	if DiscordWebhook then
		...
		local embedData = {
			["title"] = "Player Banned",
			["description"] = "**" .. player.Name .. "** (ID: " .. player.UserId .. ") has been banned by the anti-cheat.",
			["color"] = 16711680, -- Red
			["fields"] = {
				{
